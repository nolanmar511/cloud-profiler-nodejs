...
void encodeMessage(std::vector<char>& b, int tag, ProtoMessage* m) {
  std::vector<char> b1 = std::vector<char>();
  m->encode(b1);
  int len = b1.size();
  encodeLength(b, tag, len);
...
ProfFunction::ProfFunction(uint64_t id, int64_t nameX, int64_t systemNameX,
...
void ProfFunction::encode(std::vector<char>& b) {
  encodeUint64Opt(b, 1, id);
...
ProfLocation::ProfLocation(uint64_t id, uint64_t mappingIDX, uint64_t address,
...
void ProfLocation::encode(std::vector<char>& b) {
  encodeUint64Opt(b, 1, id);
...
Profile::Profile(std::string periodType, std::string periodUnit, int64_t period,
    int64_t timeNanos, int64_t durationNanos, std::string dropFrames,
    std::string keepFramesX) {
  // first index of strings must be ""
  getStringId("");

  this->periodType = ValueType(getStringId(periodType), getStringId(periodUnit));
  this->period = period;
...
uint64_t Profile::addSample(std::unique_ptr<Node>& node, std::deque<uint64_t> stack) {
...
uint64_t Profile::getLocationId(std::unique_ptr<Node>& node) {
  ...
  ProfLocation l = ProfLocation(id, 0, 0, lines, false);
...
int64_t Profile::getFunctionId(std::unique_ptr<Node>& node) {
  ...
  ProfFunction f = ProfFunction(id, nameX, nameX, filenameX, node->getLineNumber());
...
int64_t Profile::getStringId(std::string s) {
  ...
  stringIdMap.insert(stringIdMap.begin(), std::pair< std::string, int64_t>(s, id));
  strings.push_back(s);
  return id;
}

void Profile::encode(std::vector<char>& b) {
  for(std::vector<ValueType>::iterator x = sampleType.begin(); x != sampleType.end(); ++x) {
    encodeMessage(b, 1, &*x);
  }

  for(std::vector<Sample>::iterator x = sample.begin(); x != sample.end(); ++x) {
    encodeMessage(b, 2, &*x);
  }

  for(std::vector<Mapping>::iterator x = mapping.begin(); x != mapping.end(); ++x) {
    encodeMessage(b, 3, &*x);
  }
  for(std::vector<ProfLocation>::iterator x = location.begin(); x != location.end(); ++x) {
    encodeMessage(b, 4, &*x);
  }
  for(std::vector<ProfFunction>::iterator x = function.begin(); x != function.end(); ++x) {
    encodeMessage(b, 5, &*x);
  }
...
public:
  HeapNode(AllocationProfile::Node* node) {
    this->node = node;
  }
  ...
  virtual int64_t getFileID() {
    return node->script_id;
  }
  virtual int64_t getLineNumber() {
    return node->line_number;
  }
  virtual int64_t getColumnNumber() {
    return node->column_number;
  }
  virtual std::vector<Sample> getSamples(Profile& p, std::deque<uint64_t> stack) {
    std::vector<Sample> samples;
    for(std::vector<AllocationProfile::Allocation>::iterator allocation = node->allocations.begin(); allocation != node->allocations.end(); ++allocation) {
      std::vector<Label> labels = {Label(p.getStringId("allocation"), 0, allocation->size, p.getStringId("bytes"))};
      std::vector<int64_t> vals;
      vals.push_back(allocation->count);
      vals.push_back(allocation->size * allocation->count);
...
public:
  ...
  virtual std::vector<Sample> getSamples(Profile& p, std::deque<uint64_t> stack) {
    ...
    vals.push_back(hitCount);
    vals.push_back(hitCount * samplingIntervalMicros);
...
std::vector<char> serializeTimeProfile(std::unique_ptr<CpuProfile> profile, int64_t samplingIntervalMicros, int64_t startTimeNanos) {
  int64_t startTime = profile->GetStartTime();
  int64_t endTime = profile->GetEndTime();
  int64_t durationNanos = (endTime - startTime) * 1000;

  Profile p = Profile("wall", "microseconds", samplingIntervalMicros, startTimeNanos, durationNanos);
  ...
  while(entries.size() > 0) {
    ...
    std::unique_ptr<Node> node(new TimeNode(entry.node, samplingIntervalMicros));
...
std::vector<char> serializeHeapProfile(std::unique_ptr<AllocationProfile> profile, int64_t intervalBytes, int64_t startTimeNanos) {
  ...
  for(std::vector<AllocationProfile::Node*>::iterator node = root->children.begin(); node != root->children.end(); ++node) {
  HeapEntry entry;
  entry.node = *node;
  ...
  while(entries.size() > 0) {
    ...
    std::deque<uint64_t> stack = entry.stack;
    stack.push_front(loc);
    for(std::vector<AllocationProfile::Node*>::iterator node = entry.node->children.begin(); node != entry.node->children.end(); ++node) {
      HeapEntry entry;
      entry.node = *node;
      entry.stack = stack;
      entries.push_back(entry);

...
# inlining is enabled.
GCLOUD_PROFILER_LOGLEVEL=5 GAE_SERVICE={{.Service}} {{if .WantTimeLineNumbers}}GCLOUD_PROFILER_CONFIG="detailed_line_config.json"{{end}} node {{if .WantTimeLineNumbers}}--noturbo-inlining{{end}} --trace-warnings --require @google-cloud/profiler $BENCH 600
	...
	testcases := []nodeGCETestCase{
		{
			...
			wantProfiles: []profileSummary{{profileType: "WALL", functionName: "busyLoop"}, {profileType: "HEAP", functionName: "benchmark"}},
			nodeVersion:  "6",
		},
		{
			InstanceConfig: proftest.InstanceConfig{
